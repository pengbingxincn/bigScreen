...
function back() {
    if (mapStack.length != 0) {//如果有上级目录则执行
        ...
        $.get('../../static/asset/json/map/' + map.mapId + '.json', function (mapJson) {
...
function mapChart(divid) {
    ...
    $.get('../../static/asset/json/map/' + chinaId + '.json', function (mapJson) {
        chinaJson = mapJson;
        ...
        myChart.on('click', function (param) {
            ...
            if (cityId) {//代表有下级地图
                $.get('../../static/asset/json/map/' + cityId + '.json', function (mapJson) {
